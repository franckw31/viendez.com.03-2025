...
<script>
    ...
    geocoder.on('result', function(e) {
        ...
        sendDataToServer(address, coordinates);
        ...
        marker.on('dragend', async function() {
            ...
            // Send updated address and coordinates to the server
            sendDataToServer(updatedAddress, [lngLat.lng, lngLat.lat]);
        });
    });
    ...
    function sendDataToServer(address, coordinates) {
        fetch('', {
            method: 'POST',
            headers: {
    ...
    function addToHistory(address, coordinates) {
        history.push({ address, coordinates });
        updateHistoryTable();
    ...
    function updateHistoryTable() {
        ...
        history.forEach(item => {
            ...
            row.innerHTML = `<td>${item.address}</td><td>${item.coordinates[1]}</td><td>${item.coordinates[0]}</td>`;
