...
echo "------".$actu."------";
//$sql = "SELECT * FROM `activite` WHERE ($actu > strtotime(`date_depart`)) ORDER BY `date_depart` ";
//$sql = "SELECT * FROM `activite` WHERE `ville` LIKE 'Colomiers' ORDER BY `id-activite` DESC";
//$sql = "SELECT * FROM `activite` WHERE (`ville` LIKE 'Colomiers') ";

$sql = "SELECT * FROM `activite`";
//$sql = "SELECT * FROM `activite` WHERE `date_depart` <> '0000-00-00' and datediff(`date_depart`,now())>-1 ORDER BY `date_depart` ";
  //$sql = "SELECT * FROM `activite` WHERE (strtotime($maintenant) > `date_depart`) ORDER BY `date_depart` ";
...
////	echo "(".$maintenantnum.")";
////	echo "[".$dateactivitenum."]";
////	echo "/".$eventData['date_depart']."/";
        if (1) $returnData[] = [
