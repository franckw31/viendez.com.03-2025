...
if (strlen($_SESSION['id']) == 0) {
    ...
    function fetchMembres() {
        ...
        $query = "SELECT * FROM membres WHERE `id-membre` > 0";
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            $id = (int)$row['id-membre'];
            if (!$id) continue;
            
            // Initialisation des valeurs par défaut
            $row['nb_participations'] = 0;
            $row['activites'] = 'Aucune';
            $row['challenges'] = 'Aucun';
            
            // Participations et activités
            $act_query = "SELECT 
                COUNT(DISTINCT p.`id-participation`) as total,
                GROUP_CONCAT(DISTINCT a.`titre-activite`) as activites,
                GROUP_CONCAT(DISTINCT c.titre_challenge) as challenges
            ...
            if ($act_result && ($data = mysqli_fetch_assoc($act_result))) {
                $row['nb_participations'] = $data['total'] ?? 0;
                $row['activites'] = $data['activites'] ?: 'Aucune';
                $row['challenges'] = $data['challenges'] ?: 'Aucun';
            }
