...
if (strlen($_SESSION['id']) == 0) {
    ...
    function fetchMembres() {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            // Obtenir le nombre de participations
            $id = $row['id-membre'];
            $part_query = "SELECT COUNT(*) as total FROM participation WHERE `id-membre` = '$id'";
            $part_result = mysqli_query($conn, $part_query);
            $part_count = mysqli_fetch_assoc($part_result);
            $row['nb_participations'] = $part_count['total'];
            
            // Obtenir les activités
            $act_query = "SELECT DISTINCT a.`id-membre` 
            ...
            while ($act = mysqli_fetch_assoc($act_result)) {
                $activites[] = $act['id-membre'];
            }
            $row['activites'] = !empty($activites) ? implode(', ', $activites) : 'Aucune';
