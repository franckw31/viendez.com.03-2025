...
if (strlen($_SESSION['id']) == 0) {
    ...
    function fetchMembres() {
        ...
        $query = "SELECT m.`id-membre`, m.pseudo, p.points,
                    COUNT(DISTINCT p.`id-participation`) as nb_participations,
                    (SELECT GROUP_CONCAT(DISTINCT act.`titre-activite`)
                     FROM participation part 
                     JOIN activite act ON part.`id-activite` = act.`id-activite`
                     WHERE part.`id-membre` = m.`id-membre`) as activites
                 FROM participation p
                 JOIN membres m ON p.`id-membre` = m.`id-membre`
                 WHERE p.points > 0
                 GROUP BY m.`id-membre`, m.pseudo, p.points
