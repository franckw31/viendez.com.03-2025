...
if (isset($_POST['submitchoixact'])) {
    $acti = $_POST['acti'];
    $_SESSION['selected_activity'] = $acti; // Store selected activity in session
...
if (isset($_POST['delete_participant'])) {
    ...
    $sql_delete = mysqli_query($con, "DELETE FROM `participation` WHERE `id-membre` = $membre_id AND `id-activite` = $activite_id");
    if ($sql_delete) {
...
<div class="container">
        ...
        <form method="post">
            <table>
                <tr> 
                    ...
                    <td>
                        <?php
                        $acti = mysqli_query($con, "SELECT `id-activite`,`titre-activite`,`date_depart` FROM `activite` WHERE (`date_depart` >= '$actu2') ORDER BY `id-activite` DESC");
                        echo "<select name='acti' class='form-control'><option value='-Anonyme-'>Date</option>";
                        while ($choix = mysqli_fetch_assoc($acti)) {		  
                            $liste = $choix['titre-activite']; 
                            echo "<option value='{$choix["id-activite"]}'>{$choix["date_depart"]}</option>";   
                        }
                        echo "</select>";
                        ?>
                    </td>
            ...
            <table>
                <tr> 
                    ...
                    <td>
                        ...
                        echo "<select name='membre' class='form-control'><option value='-Anonyme-'>Pseudo</option>";
                    ...
                    <td colspan="2" class="btn-container">
                        <button type="submit" class="btn btn-primary-orange2 btn-block" name="submit">Inscription Rapide </button>
                    </td>
                </tr>
            </table>
        </form>
            ...
            <table>
                <tr> 
                    ...
                    <td>
                        <?php
                        $membres = mysqli_query($con, "SELECT `id-membre`,`pseudo` FROM `membres` ORDER BY `pseudo` ASC");
                        echo "<select name='membresup' class='form-control'><option value='-Anonyme-'>Pseudo</option>";
                        while ($choix = mysqli_fetch_assoc($membres)) {		  
                    ...
                    <td>
                        <?php
                        $acti = mysqli_query($con, "SELECT `id-activite`,`titre-activite`,`date_depart` FROM `activite` WHERE ( `date_depart` >= '$actu2') ORDER BY `id-activite` DESC");
                        echo "<select name='actisup' class='form-control'><option value='-Anonyme-'>Date</option>";
                        while ($choix = mysqli_fetch_assoc($acti)) {		  
                            $liste = $choix['titre-activite']; 
                            echo "<option value='{$choix["id-activite"]}'>{$choix["date_depart"]}</option>";   
                        }
                    ...
                    <td colspan="2" class="btn-container">
                        <button type="submit" class="btn btn-primary-orange2 btn-block" name="submitsupc">Suppression Rapide</button>
                    </td>
                </tr>
            </table>
        </form>
    ...
    <div class="card">
        ...
        <table class="data-table">
            ...
            <tbody>
                <?php
                $selected_activity = isset($_SESSION['selected_activity']) ? $_SESSION['selected_activity'] : null;
                
                $query = "SELECT m.`id-membre`, m.pseudo, a.`id-activite`, a.`titre-activite`, p.`id-table`, p.`id-siege`, a.date_depart 
                         FROM participation p 
                         JOIN membres m ON p.`id-membre` = m.`id-membre` 
                         JOIN activite a ON p.`id-activite` = a.`id-activite`";
                
                if ($selected_activity) {
                    $query .= " WHERE p.`id-activite` = " . intval($selected_activity);
                }
                
                $query .= " ORDER BY a.date_depart DESC";
                ...
                if (mysqli_num_rows($result) > 0) {
                    while($row = mysqli_fetch_assoc($result)) {
                        ...
                        echo "<td>
                                <form method='post' style='display:inline;'>
                                    <input type='hidden' name='membre_id' value='{$row['id-membre']}'>
                                    <input type='hidden' name='activite_id' value='{$row['id-activite']}'>
                                    <button type='submit' name='delete_participant' class='btn btn-primary-orange2' 
                    ...
                    echo "<tr><td colspan='6' style='text-align: center;'>Aucun participant trouvé pour cette activité</td></tr>";
                }
                ?>
            </tbody>
        </table>
