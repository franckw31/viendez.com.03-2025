...
<head>
    ...
    <style>
        ...
        table.dataTable tbody tr.clickable-row:active {
...
<body>
    ...
    $(document).ready(function() {
        ...
        // Gérer le clic sur les lignes pour rediriger vers la page de détails
        $('#employeeTable').on('click', 'tr.clickable-row', function(e) {
            // Ne pas déclencher le clic si on a cliqué sur un bouton ou un lien à l'intérieur
            if ($(e.target).is('button, a') || $(e.target).parents('button, a').length) {
                return;
            }
            ...
            window.location.href = 'employee-details.php?id=' + employeeId;
        });
        
        // Ajouter un effet visuel quand on survole une ligne
        $('#employeeTable tbody').on('mouseenter', 'tr.clickable-row', function() {
            $(this).css('transition', 'background-color 0.2s');
        });
        
        // Afficher un message dans la console pour indiquer que le tableau est prêt
        console.log('DataTable initialisé avec succès et lignes cliquables activées');
    });
    </script>
