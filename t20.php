...
<body>
                    ...
                    <table id="employeeTable" class="table table-hover w-100">
                        ...
                        <tbody>
                            ...
                            <tr class="clickable-row" data-id="<?= $row['id'] ?>">
                                ...
                                <td data-order="<?= strtolower($row['first_name']) ?>" class="auto-resize input-container">
                                    <input type="text" class="form-control" value="<?= ucwords(strtolower($row['first_name'])) ?>" name="first_name[<?= $row['id'] ?>]">
                                    <i class="bi bi-pencil-square edit-icon" data-id="<?= $row['id'] ?>" data-field="first_name"></i>
                                </td>
                                <td data-order="<?= strtoupper($row['last_name']) ?>" class="auto-resize input-container">
                                    <input type="text" class="form-control" value="<?= strtoupper($row['last_name']) ?>" name="last_name[<?= $row['id'] ?>]">
                                    <i class="bi bi-pencil-square edit-icon" data-id="<?= $row['id'] ?>" data-field="last_name"></i>
                                </td>
                                <td data-order="<?= $row['position'] ?>" class="auto-resize input-container">
                                    <input type="text" class="form-control" value="<?= $row['position'] ?>" name="position[<?= $row['id'] ?>]">
                                    <i class="bi bi-pencil-square edit-icon" data-id="<?= $row['id'] ?>" data-field="position"></i>
                                </td>
                                <td data-order="<?= $row['office'] ?>" class="auto-resize input-container">
                                    <input type="text" class="form-control" value="<?= $row['office'] ?>" name="office[<?= $row['id'] ?>]">
                                    <i class="bi bi-pencil-square edit-icon" data-id="<?= $row['id'] ?>" data-field="office"></i>
                                </td>
                                <td data-order="<?= strtotime($row['start_date']) ?>" class="auto-resize input-container">
                                    <input type="date" class="form-control" value="<?= $row['start_date'] ?>" name="start_date[<?= $row['id'] ?>]">
                                    <i class="bi bi-pencil-square edit-icon" data-id="<?= $row['id'] ?>" data-field="start_date"></i>
                                </td>
                                <td data-order="<?= $row['salary'] ?>" class="auto-resize input-container">
                                    <input type="number" step="0.01" class="form-control salary-cell" value="<?= $row['salary'] ?>" name="salary[<?= $row['id'] ?>]">
                                    <i class="bi bi-pencil-square edit-icon" data-id="<?= $row['id'] ?>" data-field="salary"></i>
                                </td>
    ...
    $(document).ready(function() {
        ...
        $('#employeeTable').on('click', '.edit-icon', function(event) {
            ...
            // Save the field value when the input loses focus
